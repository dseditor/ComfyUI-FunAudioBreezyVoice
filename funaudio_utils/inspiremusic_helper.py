...
class InspireMusicHelper():
    def music_create(self,task,duration,text,chorus,audio_prompt,input_sample_rate,model_name, model_dir,fast):
        if task == "continuation" :
            audio_length = audio_prompt.shape[1] / input_sample_rate
        else :
            audio_length = 1.0
        model = InspireMusicUnified(
            ...
            max_generate_audio_seconds = duration, # 生成音频的长度（秒）
